    ...
    icon: (
      ...
      >
        <path
          ...
          strokeLinejoin="round"
          strokeWidth={2}
          d="M12 4.354a4 4 0 1 0 0 5.292M15 21H3v-1a6 6 0 0 1 12 0v1zm0 0h6v-1a6 6 0 0 0-9-5.197M13 7a4 4 0 1 1-8 0 4 4 0 0 1 8 0z"
        />
      </svg>
    ),
  },
      ...
      {PARTICLE_CONFIG.map((particle, index) => (
        <span
          ...
          style={{
            ...
            opacity: 0.3,
            background: `radial-gradient(circle, ${particle.color}, ${particle.color}88, transparent)`,
            boxShadow: `0 0 ${particle.size * 4}px ${particle.color}, 0 0 ${
              particle.size * 8
            }px ${particle.color}44`,
          }}
        />
      ))}
    </div>
...
const HeroSectionComponent = () => {
  ...
  const handleMouseMove = (event: MouseEvent<HTMLDivElement>) => {
    ...
    const timerId = window.setTimeout(() => {
      ...
      starTimers.current = starTimers.current.filter(
        (timer) => timer !== timerId,
      );
    }, 650);
  ...
  return (
    <motion.section
      ...
      aria-labelledby="storyspark-hero-title"
    >
      ...
      >
        ...
        >
          ...
          <motion.h1
            id="storyspark-hero-title"
            variants={itemVariants}
            className="text-3xl sm:text-5xl lg:text-7xl font-extrabold mb-6 sm:mb-8 leading-tight select-none tracking-tight"
          >
            StorySparkAI: Create Stories Faster With{" "}
          ...
          <motion.p
            variants={itemVariants}
            className="max-w-3xl mx-auto text-sm sm:text-lg lg:text-xl text-slate-600 dark:text-slate-400 leading-relaxed mb-8 sm:mb-10 font-medium"
          >
            Turn ideas into polished stories, explore multiple plot directions,
            and collaborate with other writers in one intelligent storytelling
            workspace.
          </motion.p>
        ...
        <div className="absolute inset-0 -z-10 pointer-events-none overflow-hidden select-none">
          <div
            className="hero-cursor-stars absolute inset-0"
            aria-hidden="true"
          >
            {stars.map((star) => (
              <span
